...
### HJ6 质数因子
直接从2开始累除取余0，能被 4 整除就一定能被 2 整除，6，8，9也一样
...
cow x 行  
col y 列  
**深度优先算法**：深度优先搜索的思想是沿着一个方向搜到底，如果行不通，
则返回来试其他的路径。就一直这样直到找到一条通路输出就可以了    

**广度优先算法BFS**：广搜的套路就是利用一个队列，每次将当前点的邻居节点加入队列，然后出队，
在将当前点加入队列，一直讲所有路径搜索完毕，直到队列为空停止。
同时还需要一个数组去保存该节点是否访问，做个标记。但是怎样输出路径呢，
我采取的办法是每次我们需要保存一下当前节点的前驱节点

## HJ77
火车只有两种情况
1. 栈内有车，栈内车先出
2. 栈内无车，自己入栈
假装还有还有一辆车，执行到第二步直接跳出

## HJ50
四则运算 表达式求值
先递归到括号的最后一层，在按照常规思路算，额外定义 0 + 测试用例

## HJ69
矩阵乘法
xy yz xz  三层循环 x z y  2 3 2
数学规律
...
如num = 2.7 可以设置左边界为min(-1,2.7) = -1 右边界 max(1,2.7) = 2.7
所以left = -1，right = 2.7

## HJ103
梅花桩
动态规划：dp[i]表示 到元素i时，最长的递增子序列长度

## HJ24 需再写一遍
合唱队 出列人数
动态规划：计算当前数的左侧最大递减序列和右侧的最大递增序列    
计算出当前数字的左右递增递减序列的和，和总人数的差值即为需出栈的人数

## HJ48 
链表可以查询，也可从头尾拿，也可以指定位置

## HJ57
大整数相加，需要擅长使用断点，也可直接使用BigInteger的API

## HJ52
字符串的编辑距离，dp算法，dp[i][j] i：原数组，j：目标数组，将前2个字符转换为前j个字符需要多少步骤
两种可能，i和j的字符相同，则和i-1j-1相同，否则，替换，插入和删除，上左前三个数值都+1取最小值
...
## HJ61
分苹果，m个苹果放到n个盘子上
1. dp数组[i][j] 表示将i个苹果放到j个盘子里有多少种方法
2. 递推关系：盘子多于苹果，等于盘子少一种的放法；盘子少于等于苹果，等于盘子少一个的放法加上每个盘子一个苹果后剩下的苹果再放的放法
3. 初始化：当苹果为0时，无论多少盘子都只有一种方法
4. 遍历顺序，先盘子依次增加，再苹果依次增加
5. 打印数组来调试

## HJ90
合法IP
用.分割，正则需要使用\\.
长度需要等于4，每段长度等于0||大于3都不合法，每段都必须是数字，除0之外以0开头为0不可，每段的数小于255

## HJ64
MP3光标位置
注意两类特殊情况，第一类，歌曲数目少于4，第二类，第一个往上和最后一个往下
